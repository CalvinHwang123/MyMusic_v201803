 ...
 *
 */
public class OnlineMusicPane extends JPanel {
	...
	private void initPopMenu(MouseEvent e) {
		...
		JMenuItem collect = new JMenuItem("收藏");
		pop.add(download);
		pop.add(collect);
		...
		collect.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < subList.size(); ++i) {
					Song s = subList.get(i);
					if (songTable.getSelectedRow() == i) {
						// System.out.println("选中了" + s.toString());
						requestCollect(s.getsId());
					}
				}
			}

			private void requestCollect(int sId) {
